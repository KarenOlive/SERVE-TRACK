...
export async function POST(request) {
  ...
  try {
    ...
    const { 
      ...
      organizationName, organizationDescription, website, 
      contactPhone, address, location 
    } = await request.json();

    console.log('📝 Nonprofit registration attempt for:', { email, organizationName });
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO nonprofit_profiles 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [userId, organizationName, organizationDescription, website, contactPhone, address, location]
      );
